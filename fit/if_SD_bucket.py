...
def build_initial_guess(beta_ols, sigma2, n_buckets):
    ...
    return {
        ...
        "A":          jnp.diag(jnp.array(np.full(P_FULL, 0.05, dtype=np.float64))),
        "sigma2":     jnp.array(sigma2),
        "omega_load": jnp.array(omega_load),
        "eta":        jnp.full(P_FULL, ETA_INIT),
        "alpha":      jnp.full(P_FULL, ALPHA_INIT),
        "a_midas":    jnp.ones(P_FULL) + 0.2,
        "b_midas":    jnp.full(P_FULL, 5.0),
        "C":          jnp.diag(jnp.array(np.full(P_FULL, 1e-3, dtype=np.float64))),
        "nu":         jnp.array(10.0),
    }
...
def warm_start_guess(prev_params):
    return {
        ...
        "a_midas":    jnp.array(np.array(prev_params["a_midas"])),
        "b_midas":    jnp.array(np.array(prev_params["b_midas"])),
...
def main():
    ...
    for k in K_VALUES:
        ...
        if os.path.exists(opath) and os.path.getsize(opath) > 0:
            ...
            ws, lambdas = _solve_weights_midas(
                jnp.array(fit_output["a_midas"]),
                jnp.array(fit_output["b_midas"]),
                jnp.array(fit_output["eta"]),
                jnp.array(fit_output["alpha"])[0],
                k,
            )
            fit_output["ws"]      = np.array(ws)
            fit_output["lambdas"] = np.array(lambdas)
        else:
        ...
        a_midas_fit = np.array(fit_output["a_midas"])
        b_midas_fit = np.array(fit_output["b_midas"])
        print(f"  nu={nu_fit:.2f}, sigma2={sigma2_fit:.4f}")
        print(f"  eta={eta_fit.round(4)}")
        print(f"  alpha={float(alpha_fit.ravel()[0]):.4f}")
        print(f"  a_midas={a_midas_fit.round(4)}")
        print(f"  b_midas={b_midas_fit.round(4)}")
        print(f"  log_lik={float(np.array(fit_output['log_likelihood']).ravel()[0]):.2f}")
        print(f"  lambdas (factor 0)={fit_output['lambdas'][:, 0].round(4)}")

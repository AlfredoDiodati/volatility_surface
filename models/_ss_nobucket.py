...
def fit(
    ...
    def _link(unconstrained_params: np.ndarray) -> dict:
        ...
        bar_beta = unconstrained_params[end_B:]
    ...
    def _invlink(constrained_params: dict):
        ...
        return np.concatenate([uncH, uncQ, uncB, constrained_params["bar_beta"]])
...
def fit_collapsed(
    ...
    initial_Zt_cube = _build_Zt_cube()
...
def forecast(fit_result, M, y_test, q_alpha):
    ...
    def _step(carry, inputs):
        ...
        ZtV = Z_masked.T @ v
        Inn = np.eye(a_pred.shape[0]) + h_inv * P_pred @ ZtZt
        g = P_pred @ ZtV
        _, log_det_correction = np.linalg.slogdet(Inn)
        log_det_F = n_h * np.log(sigma2) + log_det_correction
        c = np.linalg.solve(Inn, g)
        quad = h_inv * np.sum(v ** 2) - h_inv ** 2 * ZtV @ c
        oos_ll = -0.5 * (n_h * np.log(2 * np.pi) + log_det_F + quad)
        D = np.linalg.solve(Inn, P_pred @ ZtZt)
...
def forecast_rolling_h(fit_result, M, y_test, eval_horizons):
    ...
    def _B_power(Bk, _):
        return Bk @ B, Bk @ B
    _, B_powers = jax.lax.scan(_B_power, np.eye(B.shape[0]), None, length=max_h)

...
def fit(
    ...
    def _link(theta):
        ...
        idx += p
        alpha_val = jax.nn.softplus(theta[idx]) + 1.0
        alpha = np.full(p, alpha_val)
        idx += 1
        a_midas = jax.nn.softplus(theta[idx:idx + p]) + 1.0
        idx += p
        b_midas = jax.nn.softplus(theta[idx:idx + p]) + 1.0
        idx += p
    ...
    def _invlink(params):
        ...
        unc_a = np.log(np.exp(params.get("a_midas", np.ones(p)) - 1.0) - 1.0)
        unc_b = np.log(np.exp(params.get("b_midas", np.ones(p) * 5.0) - 1.0) - 1.0)

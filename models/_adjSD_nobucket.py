...
def _filter(y_masked, base_covariates, mask_f, params, state0):
    B = params["B"]
    A = params["A"]
...
def fit(
    ...
    def _link(theta):
        ...
        return {"beta_bar": beta_bar, "B": B, "A": A, "sigma2": sigma2, "C": C_diag, "nu": nu}
    ...
    def _invlink(params):
        ...
        return np.concatenate([params["beta_bar"], unc_B, unc_A, np.array([unc_s2]), unc_C, np.array([unc_nu])])

    def _criterion(theta):
        params = _link(theta)
        _, lls, _ = _filter(y_masked, base_covariates, mask_f, params, params["beta_bar"])
        return -np.sum(lls)
    ...
    betas, _, beta_T = _filter(y_masked, base_covariates, mask_f, params_opt, params_opt["beta_bar"])
    return params_opt | {
        "betas": betas,
        "beta_T": beta_T,
...
def forecast(fit_result, M, y_test, alpha):
    ...
    betas, log_liks, _ = _filter(y_masked, base_covariates, mask_f, fit_result, state0)
...
def forecast_rolling_h(fit_result, M, y_test, eval_horizons):
    ...
    betas_origins, _, _ = _filter(
        y_masked, base_covariates[:T_half], mask_f, fit_result, fit_result["beta_T"],
    )
...
def forecast_h(fit_result, M):
    ...
    _, (predictions, P) = lax.scan(_step, fit_result["beta_T"], base_covariates)

...
def _filter(y_masked, base_covariates, mask_f, params, state0):
    ...
    (_, score_buf_T, write_idx_T), (betas, log_liks, beta_Ts) = lax.scan(
        _step, (beta0, score_buf0, write_idx0), (y_masked, base_covariates, mask_f)
    )
...
def fit(
    ...
    def _link(theta):
        ...
        return {"beta_bar": beta_bar, "A": A, "d": d, "sigma2": sigma2, "C": C_diag, "nu": nu}
    ...
    def _invlink(params):
        ...
        return np.concatenate([params["beta_bar"], unc_A, unc_d, np.array([unc_s2]), unc_C, np.array([unc_nu])])
...
def forecast_rolling_h(fit_result, M, y_test, eval_horizons):
    ...
    _, betas_h_all = lax.scan(
        _step, state0, (y_masked, base_covariates[:T_half], mask_f)
    )

    t_idx = np.arange(T_half)
    Z_h_stack = base_covariates[t_idx[None, :] + h_shifts[:, None]]
...
def forecast_h(fit_result, M):
    ...
    _, (predictions, P) = lax.scan(
        _step, (fit_result["beta_T"], np.asarray(0, np.int32)), base_covariates
    )
...
def forecast(fit_result, M, y_test, alpha):
    ...
    betas, log_liks, _, _, _ = _filter(y_masked, base_covariates, mask_f, fit_result, state0)

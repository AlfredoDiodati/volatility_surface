...
def simulate_panel(params, M, n, key, K, b_0=None, beta_tilde_0=None):
    B = params["B"]
    A = params["A"]
    sigma2 = params["sigma2"]
    sigma_0 = params["sigma_0"]
    ...
    if b_0 is None: b_0 = np.zeros(K + 1)
    ...
    def _one_path(k):
        ...
        w_samp = jax.random.normal(k2, shape=(T, p_full))
        ...
        def step(state, inputs):
            ...
            b_next = exp_neg_lambdas * b_t + weights * scaled_score[-1]
            beta_tilde_next = IminusB @ beta_bar + B @ beta_tilde_t + scaled_score[:-1]

            return (b_next, beta_tilde_next), (y_t, beta_full_t)

        _, (y_path, beta_path) = lax.scan(step, (b_0, beta_tilde_0), (g_samp, w_samp, z_samp, design))

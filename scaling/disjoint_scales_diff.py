...
def main():
    ...
    for label in data.columns:
        ...
        scaling_dict = moment_scaling(column_diff, 1.0, 126.0, moments)
        ...
        plt.ylabel(r"$S_{\Delta}(q, \Delta t)$")
        plt.xlabel(r"$\Delta t$")
        plt.xlim((dt[0], dt[-1] + 15))
        plt.xticks(rotation=0, ha="right")
        plt.savefig("plot/" + subfolder + "/put/diff_scaling" + label + ".pdf")
        plt.close()
    ...
    plt.ylabel(r"$\tau_{\Delta}(q)$")
    plt.xlabel(r"$q$")
    plt.xlim((moments[0], moments[-1]))
    plt.ylim((None, 1.0))
    plt.savefig("plot/" + subfolder + "/put/moments" + "_diff_scaling_" + ".pdf")
    plt.close()
